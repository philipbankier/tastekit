...
pnpm --filter @actrun_ai/tastekit-core exec vitest run \
...
pnpm --filter @actrun_ai/tastekit-adapters exec vitest run \
  __tests__/claude-code-contracts.test.ts

echo "[contracts] running cli contract conformance tests"
pnpm --filter @actrun_ai/tastekit-cli exec vitest run \
...
pnpm --filter @actrun_ai/tastekit-validator test
