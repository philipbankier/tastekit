...
The `@kairox_ai/tastekit-core` library includes a `Tracer` class that makes it easy to write trace events from anywhere in the system. It handles the creation of the trace file, the injection of metadata like the `run_id` and `timestamp`, and the serialization of events to the JSONL format.

```typescript
import { Tracer } from '@kairox_ai/tastekit-core';
