...
import { lintSkills } from '@kairox_ai/tastekit-core/skills';
import { packSkills } from '@kairox_ai/tastekit-core/skills';
import { analyzeSkillGraph } from '@kairox_ai/tastekit-core/skills';
import { resolveSkillsPath } from '@kairox_ai/tastekit-core/utils';
...
const skillsReportCommand = new Command('report')
  ...
  .action(async (_options, cmd) => {
    const globals = getGlobalOptions(cmd);
    const workspacePath = join(process.cwd(), '.tastekit');
    const { resolveTracesPath } = await import('@kairox_ai/tastekit-core/utils');
    const { aggregateSkillRuns, getSkillPerformance, rankByFailureRate } = await import('@kairox_ai/tastekit-core/skills');
    const { TraceReader } = await import('@kairox_ai/tastekit-core/tracing');
...
const skillsInspectCommand = new Command('inspect')
  ...
  .action(async (skillId, _options, cmd) => {
    ...
    const workspacePath = join(process.cwd(), '.tastekit');
    const { resolveTracesPath } = await import('@kairox_ai/tastekit-core/utils');
    const { aggregateSkillRuns, getSkillPerformance } = await import('@kairox_ai/tastekit-core/skills');
    const { TraceReader } = await import('@kairox_ai/tastekit-core/tracing');
    const { readdirSync } = await import('fs');
...
const skillsHistoryCommand = new Command('history')
  ...
  .action(async (skillId, _options, cmd) => {
    ...
    try {
      const { SkillVersioner } = await import('@kairox_ai/tastekit-core/skills');
