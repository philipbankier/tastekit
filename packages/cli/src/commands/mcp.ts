...
import { resolveBindingsPath } from '@kairox_ai/tastekit-core/utils';
...
const mcpInspectCommand = new Command('inspect')
  ...
  .action(async (serverName: string, _options, cmd) => {
    ...
    try {
      const { MCPClient } = await import('@kairox_ai/tastekit-core/mcp');
...
const mcpBindCommand = new Command('bind')
  ...
  .action(async (options, cmd) => {
    ...
    try {
      ...
      const { MCPClient, MCPBinder } = await import('@kairox_ai/tastekit-core/mcp');
